 ...
 */
static TRIPLO* realloc_triplo(TRIPLO *tr, int *n){
    (*n)++;
 ...
 */
static int return_id_triplo(TRIPLO *t, int indice){
    return t[indice].id;
}
 ...
 * @return 0;
 */
static int set_str_triplo(TRIPLO *t, int indice, char *str)
{
 ...
 * @param File Output file for the iterative program
 * @param queryFile Auxiliary file
 * @return 0
 */
...
/**
 * @brief Function that completes a TRIPLO with the information from a TUPLO
 * @param tr TRIPLO to complete
 * @param s_tr Size of the triplo
 * @param tu TUPLO whose info we want to add to a TRIPLO
 * @param s_tu Size of the tuplo
 * @return TRIPLO* 
 */
TRIPLO *complete_triplo_tuplo(TRIPLO *tr, int s_tr, void *tu, int s_tu)
{
    ...
    for(i = 0 ; i < s_tr; i++)
    {
        int id = return_id_triplo(tr,i);
        int ind = pesquisa_bin_tuplo(tu,0,s_tu,id);
        if(ind != -1)
        {
            char *login = return_tuplo_str(tu,ind);
            set_str_triplo(tr,i,login);
            free(login);
        }
    }
    return tr;
