...
/**
 * @brief  Function that starts the connections struct
 * @param s size
 * @return the struct with all its information
 */
CONNECTION *start_connections( int*s)
{
    int size = 0;
    CONNECTION *c = init_userConnections();
    int n = 1;
    ...
    {
        char *line2 = line; 
        int id = atoi(strsep(&line2,";"));
        for(int j = 1; j < 4; j++) strsep(&line2, ";");
        int followers = atoi(strsep(&line2,";"));
        int* followers_l = str_array(strsep(&line2,";"),followers);
        int following = atoi(strsep(&line2,";"));
        int* following_l = str_array(strsep(&line2,";"),following);
        int friend_size = followers + following;
        int *friend_list = concatena_arrays(followers_l, followers, following_l, following);
        c = add_elem_Connection(c, id, friend_list,friend_size,size,&n);
        free (followers_l);
        free (following_l);
        free (friend_list);
        size++;
...
void *add_elem_Connection(CONNECTION *uc, int id, int *idA,int amigosSize, int size, int *n){
    if((size + 1) > ((*n) *  30)) uc =realloc_usersConnections(uc,n);
