 ...
 */
static UTYPESTATS  init_types_users() {
 ...
 * @return 0
 */
static int incrementaTypes (UTYPESTATS u, int n) {
    if (n==0) u->bot++;
...
/**
 * @brief  Function that starts the typesstats related catalog
 * @return ut 
 */
UTYPESTATS start_typesstats(){
    ...
    while(fgets(line,200000,ficheiro)){
        ...
        if (strcmp(type,"Bot")==0) incrementaTypes(ut,0);
