...
int checkList (char *list, int size)
{
    ...
    int r = 1;
    if(array_list.size != size) r = 0;
    else for(i = 0; i < size && r; i++) if (array_list.array_f[i] < 0) r = 0;
    free(array_list.array_f);
    return r;
}
    ...
    for (; i < 10 && k; i++)
    {
        ...
        switch (i){
            ...
            case 7://following_list
                ...
                break;
            default:
                if(i == 9) token[strlen(token)-2] = '\0';
                if (isNumber(token)) k = checkPos(atoi(token));
                else k = 0;
                if(i == 4) followers = atoi(token);
                if(i == 6) following = atoi(token);
                break;
    ...
    for (; i < 14 && k; i++)
    {
        ...
        switch (i){
            ...
            case 9://updated_at
                ...
                break;
            default:
                if(i > 1 && i < 8) k = checkString(token);
                else
                {
                    if(i == 13) token[strlen(token)-2] = '\0';
                    if (isNumber(token)) k = checkPos(atoi(token));
                    else k = 0;
                }
                break;
...
int users()
{
    ...
    ficheiro = fopen("entrada/users-g3.csv","r");
    if(!ficheiro) printf("Não tem users-g3.csv\n");
    newFile = fopen ("guiao-1/saida/users-ok.csv","w");
    char *line = malloc(200000* sizeof (char));
    char *line2 = malloc(200000 * sizeof (char));
    if(!fgets (line, 200000, ficheiro)) return 0;
    fputs(line,newFile);
    int val = 1;
...
{
    FILE *ficheiro, *newFile; 
    ficheiro = fopen("entrada/commits-g3.csv","r");
    if(!ficheiro) printf("Não tem commits-g3.csv\n");
    newFile = fopen ("guiao-1/saida/commits-ok.csv","w");
    char *line = malloc(2000 * sizeof (char));
    char *line2 = malloc(2000 * sizeof (char));
    if(!fgets (line, 2000, ficheiro)) return 0;
    fputs(line,newFile);
    int val = 1;
    while (fgets(line,2000,ficheiro))
    {
        strcpy(line2,line);
...
{
    FILE *ficheiro, *newFile; 
    ficheiro = fopen("entrada/repos-g3.csv","r");
    if(!ficheiro) printf("Não tem repos-g3.csv\n");
    newFile = fopen ("guiao-1/saida/repos-ok.csv","w");
    char *line = malloc(2000 * sizeof (char));
    char *line2 = malloc(2000 * sizeof (char));
    if(!fgets (line, 2000, ficheiro)) return 0;
    fputs(line,newFile);
